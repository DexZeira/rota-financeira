...
export function Investments(p: ViewProps) {
  ...
  return (
    <>
      ...
      />
      </details><section aria-label="Indicadores atuais"><h2>Indicadores atuais</h2>
        <div className="market-strip">
          {([['CDI', rates.cdi], ['Selic efetiva', rates.selic], ['Meta Selic', rates.selicTarget], ['IPCA 12 meses', rates.ipca], ['TR', rates.tr]] as const).map(([label, rate]) => (
            <div className="detail" key={label}><span>{label}</span><strong>{rate ? `${dec(rate.value)}%${label === 'IPCA 12 meses' ? '' : label === 'TR' ? ' a.m.' : ' a.a.'}` : 'Indisponível'}</strong>{rate && <small>Atualizado em {new Date(rate.date + 'T12:00:00').toLocaleDateString('pt-BR')}</small>}</div>
          ))}
        </div>
      </section>
      <details className="disclosure"><summary>Expectativas Focus</summary><Card title="Expectativa Focus"><div className="inline-stats"><div className="detail"><span>Selic esperada</span><strong>{focus.selic ? `${dec(focus.selic)}%` : 'Indisponível'}</strong></div><div className="detail"><span>IPCA esperado</span><strong>{focus.ipca ? `${dec(focus.ipca)}%` : 'Indisponível'}</strong></div></div><p className="inline-note">Fonte: Relatório Focus / Banco Central · expectativa de mercado, sem uso nos cálculos históricos.</p></Card></details>
